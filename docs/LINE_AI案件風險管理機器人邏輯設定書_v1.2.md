...
為了節省 LINE 推播額度，同時確保重大風險不漏掉，後台群必須分層。

### 後台群類型

| notificationLevel | 類型 | 用途 | 即時推播 | 每日摘要 | 主動查詢 |
|---|---|---|---:|---:|---:|
| primary | 主要後台群 | 老闆或核心管理群 | high / critical | 是 | 是 |
| secondary | 一般後台群 | 設計師、工務、助理群 | critical | 是 | 是 |
| critical_only | 重大風險群 | 現場處理、維修、工務緊急群 | critical | 否 | 是 |
| test | 測試後台群 | 測試功能用 | 否 | 否 | 是 |
| none | 停用後台群 | 不接任何通知 | 否 | 否 | 否 |

### 通知原則

| 事件 | primary | secondary | critical_only | test | none |
|---|---:|---:|---:|---:|---:|
| critical 即時重大風險 | 立即推播 | 立即推播 | 立即推播 | 不推 | 不推 |
| high 高風險 | 立即推播 | 不推 | 不推 | 不推 | 不推 |
| medium 一般待審草稿 | 不即時推，進每日摘要 | 不即時推，進每日摘要 | 不推 | 不推 | 不推 |
| low 低風險 | 不推 | 不推 | 不推 | 不推 | 不推 |
| 每日 8:30 摘要 | 推播 | 推播 | 不推 | 不推 | 不推 |
| 使用者主動查詢 | 可回覆 | 可回覆 | 可回覆 | 可回覆 | 不回覆 |
| 網站新增待辦 | 不推 | 不推 | 不推 | 不推 | 不推 |
| 網站核准 AI 草稿 | 不推 | 不推 | 不推 | 不推 | 不推 |

critical 事件包含：缺失、客訴、修補、漏水、重大品質問題、客戶明顯不滿、取消案件或停工風險。這類事件必須推送所有啟用的後台通知群：`primary`、`secondary`、`critical_only`。

high 事件包含：一般高風險變更、請款 / 發票、工期可能延誤、客戶催促但未達重大風險。這類事件只即時推送 `primary`，其他後台群可在每日摘要看到。
...
```ts
{
  notificationLevel: "primary" | "secondary" | "critical_only" | "test" | "none"
}
```
...
在 LINE 群組管理頁，`notificationLevel` 下拉選單旁邊必須顯示層級說明，避免使用者不知道選項差異。

| 選項 | 說明 |
|---|---|
| 主要後台 | 收 high / critical 即時通知、每日摘要、可查詢案件。適合老闆或核心管理群。 |
| 一般後台 | 收每日摘要與 critical 即時通知、可查詢案件。適合設計師、助理、工務群。 |
| 只收重大風險 | 只收缺失、客訴、修補、漏水、重大品質問題等 critical 通知。適合現場處理群。 |
| 測試群 | 不收正式推播，只用來測試查詢與按鈕功能。 |
| 停用通知 | 不收推播，也不作為日常管理群。 |

UI 原則：

1. 下拉選單旁顯示簡短說明。
2. 使用者選到某個層級後，下方顯示目前選擇的完整說明。
3. 客戶案件群不得出現「允許 AI 回覆客戶」這類會破壞最高原則的誤導文字。
...
AI 學習規則不是讓 AI 自己改邏輯，而是讓 owner 把已確認的經驗轉成可控規則。
...
```ts
{
  ...
  targetRiskLevel: "low" | "medium" | "high" | "critical" | null
  active: boolean
...
- 只有 owner 可以新增、修改、啟用、停用 AI 學習規則。
- admin / staff 的操作只能成為參考紀錄，不可直接改變 AI 判斷。
- AI 不可以自己新增規則，也不可以自己啟用規則。
- AI 學習規則只能影響「是否建立待審草稿、草稿類型、草稿風險等級、提醒層級」。
- AI 學習規則不得覆蓋最高指導原則。
- 客戶群完全不回覆，永遠不可被學習規則覆蓋。
...
- 上班時間：4 小時未回覆提醒。
- 下班後訊息：隔天 09:30 檢查。
- critical 關鍵字：不等 4 小時，立即提醒所有啟用的後台通知群。
...
AI 學習的目的不是讓 AI 自動替公司做決定，而是把 owner 已確認的判斷偏好整理成規則，讓下一次建立草稿更準。

### 學習來源

AI 可以記錄以下事件作為「待 owner 確認的學習素材」：

1. owner 通過或拒絕 AI 草稿。
2. owner 修改草稿類型，例如把 followup 改成 complaint。
3. owner 修改風險等級，例如把 medium 改成 critical。
4. owner 修改標題、內容、負責人或期限。
5. owner 在網站建立固定判斷規則。
6. owner 在 LINE 後台群使用明確管理指令。

admin / staff 的操作可以記錄在 audit_logs，但只能作為參考，不會直接變成 AI 學習規則。

### owner 控制原則

1. 只有 owner 可以新增、修改、啟用、停用 AI 學習規則。
2. AI 可以提出「建議新增規則」，但必須由 owner 確認。
3. 未經 owner 確認的學習建議，不得影響正式判斷。
4. 每條規則都必須有啟用狀態、建立者、修改者、建立時間、修改時間。
5. 所有規則變更都要寫入 audit_logs。
...
| 類型 | 範例 | 影響 |
|---|---|---|
| 關鍵字規則 | 客戶說「缺失、修補、品質不好」 | 建立 complaint 草稿，風險 critical |
| 客戶偏好 | 某類客戶常催工期表 | 更容易建立 followup 草稿 |
| 公司用語 | 「明天跟您回覆」代表公司承諾 | 建立 promise 草稿 |
| 案件規則 | 特定案件的工期、請款、圖面更敏感 | 提高草稿風險等級 |
| 忽略規則 | 某些固定寒暄不用建立草稿 | 降低誤判 |
| 通知規則 | 某些 critical 類型要通知所有後台群 | 調整推播對象 |
...
1. 客戶群完全不回覆。
2. 客戶群不出現任何按鈕。
3. AI 只能建立待審草稿，不得直接建立正式待辦。
4. 客訴、缺失、修補、漏水等 critical 事件必須通知所有啟用後台通知群。
5. 沒有案件權限的人不能看到案件內容。
6. AI 不得自行刪除訊息、關閉任務、核准草稿。
7. AI 不得自行新增、啟用、停用學習規則。

### 建議 UI

網站建立「AI 規則中心」頁面，只開放 owner 使用。
...
13. 自己新增、啟用、停用 AI 學習規則。
14. 讓 AI 學習規則覆蓋最高指導原則。
...
- critical 事件推送 primary、secondary、critical_only。
- high 事件只推 primary。
- 測試群不接正式即時推播。
- none 不接任何推播。
...
4. 客戶群完全不回覆。
5. 後台群可接收提醒與查詢。
6. 建立主要後台群 / 一般後台群 / 重大風險群 / 測試後台群策略。
7. LINE 群組管理頁的通知層級選單必須顯示清楚說明。

### Phase 2：AI 草稿

1. 分析 LINE 訊息。
2. 建立 ai_tasks pending。
3. 根據 riskLevel 與 notificationLevel 決定推播對象。
4. 人工通過後建立 tasks。
5. 網站手動新增待辦與網站核准草稿不推 LINE。

### Phase 2.5：AI 學習規則中心

1. 建立 ai_learning_rules。
2. 建立 owner 專用 AI 規則中心頁面。
3. owner 可新增、修改、啟用、停用規則。
4. admin / staff 操作只記錄為參考，不直接改規則。
5. 規則不得覆蓋最高指導原則。
...
3. AI 能建立 pending 草稿。
4. high 風險會立即推送主要後台群。
5. critical 風險會立即推送所有啟用的後台通知群。
6. medium 風險進網站待審與每日摘要。
7. 人工通過後才會建立正式 task。
8. 網站新增待辦不推 LINE。
9. 網站核准 AI 草稿不推 LINE。
10. LINE 群組管理頁可設定 primary、secondary、critical_only、test、none，且每個選項都有清楚用途說明。
11. 客戶最後訊息超過指定時間沒人回，會產生 reminder。
12. 每日 8:30 會推送風險摘要。
13. staff 不能看到未授權案件。
14. LINE 額度不足時，網站能查到失敗原因。
15. 只有 owner 可以管理 AI 學習規則。
16. AI 學習規則不能覆蓋客戶群完全不回覆。
17. AI 學習規則只能影響待審草稿判斷與提醒層級，不得直接建立正式待辦。
